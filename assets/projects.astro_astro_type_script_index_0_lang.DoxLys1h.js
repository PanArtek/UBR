class a{constructor(){this.projects=[],this.filteredProjects=[],this.init()}async init(){if(!this.isLoggedIn()){this.showAuthError();return}document.getElementById("mainContent").style.display="block",await this.loadProjects(),this.updateStats(),this.renderProjects(),this.setupFilters()}isLoggedIn(){const s=sessionStorage.getItem("ubr_admin_logged_in"),e=sessionStorage.getItem("ubr_admin_login_time");if(!s||!e)return!1;const i=Date.now()-parseInt(e),o=8*60*60*1e3;return i<=o}showAuthError(){document.getElementById("authCheck").classList.remove("hidden"),document.getElementById("mainContent").style.display="none"}async loadProjects(){try{const e=await(await fetch("/UBR/data/projects.json")).json();this.projects=e.projects||[],this.filteredProjects=[...this.projects],document.getElementById("loadingState").style.display="none"}catch(s){console.error("Error loading projects:",s),this.projects=[],this.filteredProjects=[],document.getElementById("loadingState").style.display="none"}}updateStats(){const s=this.projects.length,e=this.projects.filter(i=>i.isVisible).length,t=this.projects.reduce((i,o)=>i+(o.images?.length||0),0);document.getElementById("totalProjects").textContent=s,document.getElementById("visibleProjects").textContent=e,document.getElementById("totalImages").textContent=t}setupFilters(){const s=document.getElementById("categoryFilter"),e=document.getElementById("statusFilter");[s,e].forEach(t=>{t&&t.addEventListener("change",()=>this.applyFilters())})}applyFilters(){const s=document.getElementById("categoryFilter")?.value,e=document.getElementById("statusFilter")?.value;this.filteredProjects=this.projects.filter(t=>{const i=!s||t.category===s,o=!e||e==="visible"&&t.isVisible||e==="hidden"&&!t.isVisible;return i&&o}),this.renderProjects()}renderProjects(){const s=document.getElementById("projectsList"),e=document.getElementById("emptyState");if(this.filteredProjects.length===0){s.innerHTML="",e.classList.remove("hidden");return}e.classList.add("hidden"),s.innerHTML=this.filteredProjects.map(t=>`
          ...
          <div class="flex flex-col md:flex-row justify-between items-start gap-4">
            <div class="flex-1 min-w-0">
              ...
              <div class="flex flex-wrap gap-1">
                ${(t.features||[]).slice(0,3).map(i=>`<span class="bg-blue-50 text-blue-800 text-xs px-2 py-1 rounded">${i}</span>`).join("")}
      ...
      `).join("")}editProject(s){window.location.href=`/UBR/admin/edit/?id=${s}`}toggleVisibility(s){const e=this.projects.find(t=>t.id===s);e&&(e.isVisible=!e.isVisible,this.updateStats(),this.renderProjects(),this.showNotification(`Projekt "${e.title}" został ${e.isVisible?"opublikowany":"ukryty"}`,"success"))}deleteProject(s){const e=this.projects.find(t=>t.id===s);e&&confirm(`Czy na pewno chcesz usunąć projekt "${e.title}"?

Ta operacja jest nieodwracalna.`)&&(this.projects=this.projects.filter(t=>t.id!==s),this.applyFilters(),this.updateStats(),this.showNotification(`Projekt "${e.title}" został usunięty`,"success"))}showNotification(s,e="info"){const t=document.createElement("div");t.className=`fixed top-4 right-4 z-50 px-6 py-3 rounded-lg shadow-lg text-white transform transition-all duration-300 translate-x-full ${e==="success"?"bg-green-600":e==="error"?"bg-red-600":"bg-blue-600"}`,t.textContent=s,document.body.appendChild(t),setTimeout(()=>{t.classList.remove("translate-x-full")},100),setTimeout(()=>{t.classList.add("translate-x-full"),setTimeout(()=>{document.body.removeChild(t)},300)},3e3)}logout(){confirm("Czy na pewno chcesz się wylogować?")&&(sessionStorage.removeItem("ubr_admin_logged_in"),sessionStorage.removeItem("ubr_admin_login_time"),window.location.href="/UBR/admin/")}}window.showAddForm=()=>{window.location.href="/UBR/admin/edit/"};window.editProject=n=>{window.projectsManager.editProject(n)};window.toggleVisibility=n=>{window.projectsManager.toggleVisibility(n)};window.deleteProject=n=>{window.projectsManager.deleteProject(n)};window.logout=()=>{window.projectsManager.logout()};document.addEventListener("DOMContentLoaded",()=>{window.projectsManager=new a});
